...
class LinkedList{
	...
	print_list()
	{
		...
		while(tmp)
		{
			console.log(tmp.data.depart+" "+tmp.data.arrivee+" "+tmp.data.prix+" "+tmp.data.duree);
			tmp=tmp.next;
		}
	}
...
var pile;
pile= new LinkedList();//On crée une pile
pile.read();//On y stocke nos données
console.log("La taille de la pile : "+pile.size_of_list()+"\n");//On affiche la taille de la liste
pile.print_list();//On affiche la liste
var d="Beatitude";
console.log("\nLe resultat de la recherche de "+d+" est "+pile.search_elt(d));//On affiche le resultat de la recherche du depart contenu dans d
console.log("\nLa taille de la pile : "+pile.size_of_list());
pile=pile.sort_list();//On effectue le tri de la liste
pile.print_list();//On affiche la liste triée
console.log("\nLe trajet de cout minimum est : \n"+pile.minimum().depart+" "+pile.minimum().arrivee+" "+pile.minimum().prix+" "+pile.minimum().duree);//On affiche le trajet qui a le prix minimum
